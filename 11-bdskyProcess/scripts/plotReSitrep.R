plotReSitrep <- function(parameters) {
  ...
  plot <- ggplot() +
    geom_line(region.sitrep, mapping = aes(x=as.Date(Date,"%d-%b-%y"), y=Median.R.), color = "dark red", size = 1) +
    ...
    geom_line(data, mapping = aes(x=as.Date(date,"%Y-%m-%d"), y=median), color = "dark blue", size = 1) +
