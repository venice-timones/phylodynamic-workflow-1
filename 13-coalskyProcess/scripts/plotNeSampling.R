...
plotNeSampling <- function(parameters) {
  ...
  region.metadata <- do.call(rbind.data.frame, parameters[5])
  
  # Plot Ne vs. Sampling dates
  region.metadata <- data.frame(table(region.metadata$date))
  region.mult <- max(region.metadata$Freq)/max(region.file$upper, na.rm = TRUE)
  plot <- ggplot() +
    geom_col(region.metadata, mapping = aes(x=as.Date(Var1, format="%Y-%m-%d") , y=Freq), width = 1, alpha = 0.7) +
    ...
    labs(x = "Month and Year",
         y = "Reported Cases",
         title = paste0("Effective Population size vs. Sampling Dates (", title, " - 2021)")) +
